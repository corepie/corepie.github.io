 ...
 */
@WebServlet("/outcall2.do")
public class outcall2 extends HttpServlet {
		...
		try {
			...
			while (rs1.next()) {
				...
				response.setContentType("text/html; charset=UTF-8");
				 response.getWriter().print("<script language='javascript'>" +"alert('Insufficient account balance,please reselect!');"
				+ "window.location.href='http://localhost:8080/guahao/waike/waike.jsp'';</script>')");
			...
			Boolean flag = statement2.execute(sql);
			
			//实时更新用户表中的余额情况
			String sq2 ="update  用户表2 set 账户余额 =  " + yue  +"-"+ fy+ " where 身份证号 = '"+sfid+"'";
			
			Boolean flag2 = statement2.execute(sq2);
			
			//语句三，将医生表里的余号-1
			String sql33 = "select * from 医生表 where 姓名 = '"+ ys +"'";
			ResultSet rs33 = statement.executeQuery(sql33);
			while (rs33.next()){
			...
			Boolean flag3 = statement2.execute(sql2);
			}
