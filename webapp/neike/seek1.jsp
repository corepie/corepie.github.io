...
try{
	...
	while (rs.next()){
		...
		%>
			...
			<td class="quchu"><a href='../neike/seek3.jsp?ygname=<%=ygname %>'>选择</a></td>
			</tr>
...
String query2="SELECT * FROM 预约表 where 确诊人 = '"+useridsss+"' and 医生科室 = '"+nkname2+"' order by 预约编号 desc";
...
try{
	...
	out.println("<script> qufen='"+qufen+"'</script>");
...
%>	
	...
	var tabletr =document.getElementsByTagName("tr");  //获取body中所有的tr
		...
		for(var i=0;i<tabletr.length-1;i++){
			...
			ysname11 = td.innerText;
			   
			
			   //遍历预约表中的医生数量
			for (var j=1; j<=x1.length-1;j++){
			if(td.innerText == x1[j] && y1[j] =='预约成功' &&z1[j] == '1'){   //如果body中存在的医生与预约表有相同的医生，就改变格式span,并且变成红色字体
					var s2 = "tr:nth-child("+(j)+")>td:nth-child(6)";   //获得操作行
					console.log("s2: "+s2);
